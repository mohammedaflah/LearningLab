...
for noisei=1:2
    ...
    % FFT
    if noisei==1
        f = fft(sum(sine_waves)+littleNoise)/pnts ;
    else
        f = fft(sum(sine_waves)+lotsOfNoise)/pnts ;
    end
    ...
    set(gca,'xlim',[0 35],'ylim',[0 max(amplit)*1.2]) % is this a good choice for x-axis limit?!?!?!
...
powspectSeparate = mean(2*abs(powspectSeparate) .^2,2);


% now average first, then take the FFT of the trial average
powspectAverage  = (2*abs(fft(mean(csd(chan2use,:,:),3))/pnts )).^2;
...
subplot(212)
plot(hz,(2*abs(signalX(1:length(hz)))).^2/npnts,'k')
set(gca,'xlim',[0 80])
xlabel('Frequency (Hz)'), ylabel('Amplitude')
...
%  this became complex valued becuse weonly removed positive component in
%  it
%%% QUESTION: Why doesn't this procedure get rid of the line component noise?!?!?!?
% because negative component of 50 hz is still there.
...
% zero out the 50 Hz component
signalXf(hz50idx) = 0;
signalXf(end-hz50idx)=0; % hint: the negative frequencies
