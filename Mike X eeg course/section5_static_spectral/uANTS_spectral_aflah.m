...
mag = abs(compnum); % or sqrt(real(compnum)^2 + imag(compnum)^2) or sqrt(compnum * conj(compnum))
phs = angle(compnum);
...
%   and a series of sine waves. this is not acually fourier transform but
%   it takes us near to fourier transform
...
% If we multiplied by 2 the theta, we will get vaue of theta as pi/2 which will give dot product plot as strait horixontal and if we multiplied by 3 then theta  
% value will be 3pi/2which will plot as inverted.
...
% in complex valued sine wave, the dot product is independent of phase of
% the signal but in real valued sine wave its dependent.
...
xlabel('Frequency (Hz)')
ylabel('Amplitude (\muV)') %power is square of amplitude 
...
amp1 = sqrt(real(fCoefs1) .^2 +imag(fCoefs1) .^2 );% hint: use the functions "imag" and "real"
amp2 = sqrt(real(fCoefs2) .^2 +imag(fCoefs2) .^2 );
amp3 = sqrt(real(fCoefs3) .^2 +imag(fCoefs3) .^2 );
...
% sinewave became slower and slower thereby decreasing frequency
%%% QUESTION: What happens to the sine waves as they approach f=.5?
% Sinewave became faster and faster as they approach nyquist frequency(.5)
...
% Yes fCoefs are sums of dot products, so their magnitude scales with the number of points (pnts).
%The normalization step (fCoefs / pnts) adjusts for this dependency to some extent, but if improperly scaled, the amplitude can still be influenced by the signal length
...
%Compute amplitudes
ampl1 = abs(signalX1(1:length(hz)));
ampl1(2:end) = 2*ampl1(2:end);
ampl2 = abs(signalX2(1:length(hz)));
ampl2(2:end) = 2*ampl2(2:end);
ampl3 = abs(signalX3(1:length(hz)));
ampl3(2:end) = 2*ampl3(2:end);
% plot signals in the frequency domain
subplot(212)
plot(hz,ampl1,'bo-','linew',2,'markersize',8), hold on
plot(hz,ampl2,'rd-','linew',2,'markersize',8)
plot(hz,ampl,'k*-','linew',2,'markersize',8)
...
%             (Yes, of course you can! Do it!)... yes i did it
...
% power spectrum decreases with increasing function
% Very very narrow spikes at line noise frequencies
% deviation from 1/f
%%% QUESTION: What do you notice about the difference between the amplitude
%             and power spectra?
% Amplitde spectrum shows subtle features of spectrum and power spectrum
% highlights prominent features of the signal they help us to focus
% different aspects of signal
% 
...
signalX2 = fft(signal,length(signal)+10)  / length(signal);   % zero-pad to 10 plus length of signal!
...
hold on
plot(hz,2*abs(signal1X),'bs-','markerface','k','linew',2)
