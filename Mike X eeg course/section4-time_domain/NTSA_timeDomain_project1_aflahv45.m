...
% time window for negative peak and positive peak
negpeaktime = [  50 110 ];
pospeaktime = [ 110 170 ];
...
erpMin = mean(erpMinTime-win:erpMinTime+win);
erpMax = mean(erpMaxTime-win:erpMaxTime+win);
...
% now find average values around the peak time
erpMin = mean(erpFilt(erpMinTime-win:erpMinTime+win));
erpMax = mean(erpFilt(erpMaxTime-win:erpMaxTime+win));

% adjust ERP timings
erpMinTime = EEG.times(erpFMinTime);
erpMaxTime = EEG.times(erpFMaxTime);
% get results (peak-to-peak voltage and latency)
erpFP2P = erpFMax - erpFMin;
erpFP2Plat = erpFMaxTime - erpFMinTime;
...
%% done.
% Result for peak to peak point: 
% unfiltered erp (11.27 muV,66.4 ms span)
% filtered erp (8.05 muV, 58.6ms span)

% Results for window around peak
%unfiltered erp (7.9 muV,66.4 ms span)
% filtered erp (7.4 muV, 58.6ms span)
