...
%{
contourf: This is typically used for 3D data visualized in a 2D plot. You need a matrix (or grid) of values that represent how data varies across two independent variables.
Example: contourf(X, Y, Z) where X and Y are 2D matrices that define the grid, and Z is a matrix of values at each grid point.
plot: This is used for 2D data, where you have a single set of x-values and y-values.
Example: plot(x, y) where x and y are 1D arrays. %}
%% done.
